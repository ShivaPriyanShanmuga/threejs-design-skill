...
five material archetypes. Five parts, one per archetype, and a picker that swaps the body between
all of them under the same light.

![A product viewer: brushed metal body, iridescent collar, transmission-glass dome, sheen band, clearcoat base](demo-viewer/docs/brushed.png)

The model is generated by a script rather than downloaded, so the repo owns its asset outright and
there is no licence to track. It exercises `GLTFLoader` with Draco, per-part material assignment by
mesh name, `frameloop="demand"`, adaptive quality, and explicit disposal. It also produced the
clearest single lesson of the three: `gltf-transform optimize` shrank the model 8.7x, but Draco's
decoder is a fixed ~250 KB fetch, so compressing one small model **cost 90 KB net**. That
break-even is now in `performance.md`.
...
```bash
cd demo-keyboard && npm install && npm run dev   # or: demo-viewer, or demo
```
...
- **`three/addons/misc/Timer.js` no longer resolves** — `Timer` moved into the core namespace, and
  ...
  far that the skill *introduced* rather than inherited, and only running its output caught it.
