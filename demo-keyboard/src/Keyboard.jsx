...
import { ATLAS_CELL, atlasOffset, buildGlowTexture, buildLogoAtlas } from './atlas'
import { KEY, KEY_H, PITCH, TAPER, keyPosition, makeKeycapGeometry } from './keycap'
...
export default function Keyboard({ progressRef, onSelect, selected }) {
  const caps = useRef()
  const decals = useRef()
  const glow = useRef()
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      const matches = TECH.map((t, i) => [t, i]).filter(([t]) => t.title.toLowerCase().startsWith(ch))
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < N; i++) {
      const target = i === selected ? LIFT_PRESS : i === hovered ? LIFT_HOVER : 0
      // A key should snap down and ease back — lambda 14 down, 8 up.
      anim[i].y = THREE.MathUtils.damp(anim[i].y, target, target < anim[i].y ? 14 : 8, d)
    ...
    g.material.opacity = THREE.MathUtils.damp(g.material.opacity, gTarget, 9, d)
  ...
  return (
    <group>
      ...
      <RoundedBox
        ...
        receiveShadow
      >
        <meshStandardMaterial color="#191b22" roughness={0.62} metalness={0.15} />
      </RoundedBox>
      ...
      >
        <meshStandardMaterial roughness={0.38} metalness={0.0} envMapIntensity={0.45} />
      </instancedMesh>
      ...
      <SceneText progressRef={progressRef} selected={selected} />
    </group>
  )
}
 ...
 */
function SceneText({ progressRef, selected }) {
  const group = useRef()
  ...
  useFrame((_, delta) => {
    ...
    group.current.children.forEach((c) => {
      if (c.material) c.material.opacity = THREE.MathUtils.damp(c.material.opacity, target, 6, Math.min(delta, 0.1))
    })
  })
  ...
  return (
    <group ref={group} rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, 0]}>
      ...
      <Text
        position={[-SPAN * 0.68, SPAN * 0.82, 0]}
        fontSize={0.66}
        color="#eef1f8"
        anchorX="right"
        anchorY="middle"
      ...
      <Text
        position={[-SPAN * 0.68, SPAN * 0.82 - 1.0, 0]}
        fontSize={0.235}
        color="#8d94a8"
        anchorX="right"
        anchorY="top"
        ...
        material-toneMapped={false}
      >
        {tech ? tech.blurb : 'hover a key — or press one'}
      </Text>
    </group>
  )
}
